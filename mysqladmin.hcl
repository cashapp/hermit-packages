...
version "8.0.36" {
  ...
  platform "linux" "amd64" {
    ...
    on "unpack" {
      run {
        cmd = "/bin/bash"
        args = ["-c", "mkdir bin; if [ $(grep DISTRIB_RELEASE= /etc/lsb-release | cut -d= -f2 | cut -d. -f1) -ge 22 ]; then mv mysqladmin-22.04 bin/mysqladmin; else mv mysqladmin-20.04 bin/mysqladmin; fi"]
      }
    }
  }
}
