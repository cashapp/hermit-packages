...
platform darwin arm64 {
  ...
  on unpack {
    rename { from = "${root}/opam-${version}-arm64-macos" to = "${root}/opam" }
  }
}
...
platform darwin amd64 {
  ...
  on unpack {
    rename { from = "${root}/opam-${version}-x86_64-macos" to = "${root}/opam" }
  }
}
...
platform linux amd64 {
  ...
  on unpack {
    rename { from = "${root}/opam-${version}-x86_64-linux" to = "${root}/opam" }
  }
}
